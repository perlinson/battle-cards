  ...
  <v-container fluid pt-16>
    <v-row justify="center">
      <v-card class="elevation-12" outlined width="1000">
        ...
        <!-- Data Table -->
        <v-data-table :headers="headers" :items="games" :search="search" dense>
          <!-- CheckBox -->
          ...
          <!-- <template v-slot:item.action="{ item }">
              <v-btn text @click="handleActionClick(item)" color="primary">
    ...
    <v-dialog v-model="createDialog.visible" persistent max-width="500px">
      <v-card>
        <v-card-title class="blue"> Create Game </v-card-title>
        <v-card-text>
          <v-text-field
...
export default {
  ...
  methods: {
    ...
    async joinGame() {
      ...
      let rest = await this.$store.dispatch('joinGame', {
    ...
    deleteItems() {
      ...
      //     `${plural[0]}: [ ` +
      //     NotDeleted.toString() +
      //     ` ]. ${plural[1]} not Finished yet, You are unable to Delete ${plural[2]}.`
